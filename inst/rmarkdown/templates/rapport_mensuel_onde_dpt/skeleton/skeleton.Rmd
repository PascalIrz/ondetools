...
Le **département `r params$code_departement`** est couvert par **XX stations** d'observation. Pour chaque campagne usuelle d'observation, un indice ONDE peut être calculé selon la formule suivante :
...
```{r map dpt, results='asis', echo=FALSE, crop=T, fig.width=5.5, fig.height=5.5, dpi=300}

date_proche <- 
  as.Date(paste0(params$mois_rapport,'_28'),format = "%B %Y_%d")

onde_df_sel <-
  params$onde_df %>%
  dplyr::mutate(DtRealObservation = as.Date(DtRealObservation)) %>%
  dplyr::filter(CdSiteHydro %in%
                  ({# selection des dernières stations actives
                    params$onde_df %>%
                      dplyr::filter(CdDepartement == params$code_departement) %>%
                      dplyr::filter(Annee == params$annee_rapport) %>%
                      dplyr::filter(TypeCampObservations == 'usuelle') %>%
                      dplyr::mutate(DtRealObservation = as.Date(DtRealObservation)) %>%
                      dplyr::group_by(CdSiteHydro, LbSiteHydro) %>%
                      dplyr::filter(DtRealObservation == dplyr::last(DtRealObservation)) %>%
                      dplyr::pull(CdSiteHydro)})
  ) %>%
  dplyr::filter(DtRealObservation < date_proche) %>% 
  dplyr::mutate(diff_date = abs(date_voulue - DtRealObservation)) %>%
  dplyr::group_by(CdSiteHydro) %>%
...
```{r map dpt 2, results='asis', echo=FALSE, crop=T, fig.width=5.5, fig.height=5.5, dpi=300}
produire_carte_statique(code_departement = params$code_departement,
                        onde_df_mois = onde_df_sel,
