...
#'
#' @param onde_df un tableau de données onde
#'
...
calculer_indice_onde <- function(onde_df = NULL){
  {{onde_df}} %>%
    ...
    dplyr::summarise(
      ...
      nb_ecoul_cont = sum(grepl("Ecoulement visib.", libelle_ecoulement)),
      nb_ecoul_interr = sum(grepl("Ecoulement non", libelle_ecoulement)),
      nb_NA = sum(grepl("Observation impossible|Donn\u00e9e manquante", libelle_ecoulement)),
      indice = if_else(nb_NA == 0, round(((5 * nb_ecoul_interr + 10 * nb_ecoul_cont)/ nb_sta), 2), NA)
    )
}
